...
data = [
    ...
    {
        "specialization": "pediatric_dentist",
        "dentists": [
            {"name": "sarah wilson"}
        ]
    },
    {
        "specialization": "emergency_dentist",
        "dentists": [
            {"name": "daniel miller"},
            {"name": "susan davis"}
        ]
...
def generate_csv(filename):
    ...
    for day in range(30):  # Covering one month
        ...
        if date.weekday() < 5:  # Monday to Friday
            time_slots += generate_time_slots(datetime(date.year, date.month, date.day, 8, 0),
                                               datetime(date.year, date.month, date.day, 17, 0), 30)
        elif date.weekday() == 5:  # Saturday
            time_slots += generate_time_slots(datetime(date.year, date.month, date.day, 9, 0),
                                               datetime(date.year, date.month, date.day, 13, 0), 30)
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(["date_slot", "specialization", "doctor_name", "is_available"])
        ...
        for specialization in data:
            for dentist in specialization["dentists"]:
                for slot in time_slots:
                    ...
                    writer.writerow([slot, specialization["specialization"], dentist["name"], is_available])
